 ...
 *
 * The Python layer validates arrays and builds layouts and pole tables.
 * Operators here use scalar cyclic momentum arithmetic; other layouts use
 * Python. Broadening consumes pole labels and supports all momentum layouts.
 */
...
#pragma omp parallel for schedule(dynamic) default(none) \
        ...
        for (int ipole = 0; ipole < npoles; ipole++) {
                ...
                size_t base = (((size_t)k * norb_axis + p) * spin_axis + s)
                        * nomega;
...
void FCIspectral_apply_k_op(double complex *out,
                            ...
                            int beta_phase)
{
        /* Retained for compatibility with the source branch's native ABI. */
        (void)target_block_na;
/* A fixed operator maps source blocks to distinct target blocks and is
 * injective on surviving strings, so no threads share output coefficients. */
                ...
                if (spin == 0) {
                        ...
                        for (int ia = 0; ia < na; ia++) {
                                ...
                                for (int ib = 0; ib < nb; ib++) {
                                        out[off1 + (size_t)ia1 * nb1 + ib] +=
                                                (double)sign1 *
                                                fcivec[offset + (size_t)ia * nb + ib];
                                }
                        }
                } else {
