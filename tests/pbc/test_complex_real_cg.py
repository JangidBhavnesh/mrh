...
class KnownValuesRealLinearSolvers(unittest.TestCase):
    ...
    def test_cg_solves_positive_definite_hessian(self):
        # One complex variable is two real variables.  This positive-definite
        # matrix gives the two parts different curvatures and couples them.
        # The test checks that CG solves both parts together and returns the
        # answer in the original complex form.
    ...
    def test_minres_solves_indefinite_hessian(self):
        ...
        # CG is not suitable for this case.  The test checks that the MINRES
        # child class solves the same coupled real/imaginary equation.
        hessian_real = np.array([
            [2.0, 1.0],
            [1.0, -1.0],
        ])
