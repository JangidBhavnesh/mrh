...
def plot_bands(cell, e_kn, filename="h1D_bands.png", title=r"H$_2$ Band Structure"):
    # Restricted HF Only
    au2ev = 27.21139
...
if __name__ == "__main__":
    ...
    for i in range(1, 22, 6):
        kmesh = (i, 1, 1)
        h1D = make_h2_1D(intraH=0.74, interH=1.5, nx = 1, vacuum=17.5)
        cell = get_cell(h1D)
        kmf = run_scf(cell, kmesh=kmesh)
        print(f"k-point-{i} energy:", kmf.e_tot)
        # band_kpts = cell.make_kpts(kmesh, wrap_around=True)
        # e_kn = kmf.get_bands(band_kpts)[0]

        # plot_bands(cell, e_kn, filename="h1D_bands.png", title=rf"H$_2$ Band Structure {kmesh}")
