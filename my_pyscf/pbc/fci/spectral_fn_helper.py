...
The workflow is compute_kcasci_spectral_roots -> make_spectral_poles ->
make_spectral_function. Frequencies and broadening widths are in Hartree;
...
def make_k_sector_layout(norb, nelec, nkpts, target_k=0,
    ...
    if link_index is None:
        link_index = kfci_contract_map._unpack_contract_link_index(
...
def _collect_charged_roots(kmc, kind):
    ...
    for result in kmc.charged_results:
        energies_supercell = np.asarray(result.get(
            'e_tot_supercell', np.asarray(result['e_tot']) * result['nkpts']))
        for root, energy, ci in _iter_roots(result['e_tot'], result['ci']):
            roots.append({
                ...
                'root': int(root),
                'energy': energy,
                'energy_supercell': energies_supercell.reshape(-1)[root].item(),
                'ci': ci,
...
def _prepare_kcasci_job(kmc, kmom, nroots, solver_setup, kind, mo_coeff):
    '''
    Apply the common momentum and solver settings to one spectral KCASCI job.
    '''
    # Isolate each job from mutations to another job's orbitals or tables.
    kmc.mo_coeff = np.array(mo_coeff, copy=True)
    kmc.kmom = deepcopy(kmom)
    kmc.kconserv = kmom.kconserv.copy()
    kmc.kpts = deepcopy(kmc.kpts)
    kmc.fcisolver.kmom = deepcopy(kmom)
    kmc.fcisolver.kconserv = kmom.kconserv.copy()
    kmc.fcisolver.kpts = deepcopy(kmc.fcisolver.kpts)
    kmc.canonicalization = False
    if nroots is not None:
        if int(nroots) != nroots or nroots < 1:
            raise ValueError('nroots must be a positive integer')
        kmc.fcisolver.nroots = int(nroots)
    fixed_names = ('ncas', 'nelecas', 'ncore', 'nkpts', 'target_k',
                   'charge', 'charged_spin', 'kpts')
    fixed = {name: deepcopy(getattr(kmc, name, None)) for name in fixed_names}
    solver_fixed = {name: deepcopy(getattr(kmc.fcisolver, name, None))
                    for name in ('nkpts', 'target_k', 'kpts')}
    if solver_setup is not None:
        solver_setup(kmc, kind)

    def invalid(name):
        raise ValueError(f'{kind} spectral job: solver_setup must preserve '
                         f'the common orbital basis and sectors ({name})')

    if kmc.canonicalization or kmc.natorb:
        invalid('canonicalization=False and natorb=False are required')
    if not np.array_equal(kmc.mo_coeff, mo_coeff):
        invalid('mo_coeff')
    for name, expected in fixed.items():
        if not np.array_equal(getattr(kmc, name, None), expected):
            invalid(name)
    for name, expected in solver_fixed.items():
        if not np.array_equal(getattr(kmc.fcisolver, name, None), expected):
            invalid('fcisolver.' + name)
    for obj in (kmc, kmc.fcisolver):
        if not np.array_equal(getattr(obj, 'kconserv', None), kmom.kconserv):
            invalid('kconserv')
        actual = getattr(obj, 'kmom', None)
        for name in ('nkpts', 'zero', 'scalar', 'kconserv', 'kadd', 'ksub', 'kneg'):
            if not np.array_equal(getattr(actual, name, None), getattr(kmom, name)):
                invalid('kmom.' + name)
        mesh = getattr(obj, 'kmesh', None)
        if mesh is not None:
            mesh = np.asarray(mesh)
            if (mesh.shape != (3,) or np.any(mesh <= 0)
                    or not np.all(mesh == mesh.astype(int))
                    or np.prod(mesh) != kmc.nkpts):
                invalid('kmesh')
            mesh_kpts = kmc.cell.make_kpts(mesh.astype(int), wrap_around=True)
            delta = kmc.cell.get_scaled_kpts(mesh_kpts - np.asarray(kmc.kpts))
            if not np.allclose(delta, np.rint(delta), atol=1e-8, rtol=0):
...
def compute_kcasci_spectral_roots(kmf, ncas, nelecas, ncore=None,
    ...
    separately. Summing representative sectors does not reconstruct omitted
    ...
    _prepare_kcasci_job(kmc_neutral, kmom, nroots_neutral, solver_setup,
                        'neutral', mo_coeff)
    kmc_neutral.kernel(mo_coeff=kmc_neutral.mo_coeff, verbose=verbose)
    ...
    if with_hole:
        for charged_spin in _charged_spin_list(
            ...
            _prepare_kcasci_job(kmc_hole, kmom, nroots_hole, solver_setup,
                                'hole', mo_coeff)
            kmc_hole.kernel(mo_coeff=kmc_hole.mo_coeff, verbose=verbose)
    ...
    if with_particle:
        for charged_spin in _charged_spin_list(
            ...
            _prepare_kcasci_job(kmc_particle, kmom, nroots_particle,
                                solver_setup, 'particle', mo_coeff)
            kmc_particle.kernel(mo_coeff=kmc_particle.mo_coeff, verbose=verbose)
...
def make_spectral_function_py(poles, omega_grid=None, eta=0.05,
    ...
    Frequencies and eta are in Hartree; A has units of inverse Hartree.
